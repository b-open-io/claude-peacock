...
# Uses OSC 6 sequence for tab color (iTerm2-specific)
# Only set if we have a project with Peacock colors and we're in iTerm2
if [[ "$TERM_PROGRAM" == "iTerm.app" && -n "$CWD_R" && -n "$CWD_G" && -n "$CWD_B" ]]; then
